...
| forensic-v2 | _(pending suite)_ | | | | | SRI/vendor CSS + anti-flatten (layered chrome, column inference, residual padding) |

\* forensic-v1 also expanded corpus 42 → 100 pages; compare multi-metric fields, not only composite.

**Stop rule:** keep only changes that improve avg composite **> 0.5%**.

Net vs early baseline: **+7.4pp** composite (81.7 → 89.1), geometry **+9.8pp**, pixel **+9.4pp**.
